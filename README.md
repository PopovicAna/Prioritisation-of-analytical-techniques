...
agencies to work collaboratively to combat the supply and demand of
these drugs. Forensic science provides a means of determining the
presence and quantity of illicit substances in specimens by extracting
different drug profile types. These drug profile types are extracted
using analytical techniques such as gas chromatography–mass spectrometry
(**GCMS**), isotope-ratio mass spectometry (**IRMS**) and capillary
electrophoreisis (**CE**), to name a few \[1\]. Collectively these
profile types provide a very accurate representation of a drug specimen.
However, many questions have been raised recently by law enforcement and
the judicial system regarding the determination of an association or
similarity between drug specimens, to identify patterns or trends across
them \[2\].

One issue with ansering these questions is that it is often time
...
determine the level of similairty between profiles, different comparison
metrics (**CM**) are commonly applied to pre-treated (**PT**) data
\[3\].

The analytical technique that should be conducted in priority should be
able to produce the lowest false positive (FP) and false negative (FN)
rates when it comes to determining the similarity between **linked**
(specimens from the same seizure) and **unlinked** (specimens from
different seizures) specimens, see Figure 1 for an example. To clarify,
optimising the combination of CMs and PTs will ensure the greatest
separation between linked and unlinked populations, in turn minimising
FP and FN specimen linkages for each profile type. These measures need
to be compared across profile types to determine which one exhibits the
greatest discrimination between linked and unlinked specimens.

![](Files/Populations.png)

*Figure 1: An example of the distribution of linked and unlinked
specimen scores; orientation of linked and unlinked populations may vary
with the comparison metric used*

To quantify the extent of FPs and FNs for each profile type, the area
under the receiver operating characteristic curve (**AUC ROC**) is
calculated for multiple combinations of PT and CM. The optimal
combination of PT and CM will produce the largest AUC ROC (i.e. closest
to 1). The optimal combination of PT and CM is compared across
analytical techniques to determine which technique provides the most
discrimination between illicit drug profiles, hence allowing techniques
to be prioritised based on their level of discrimination power, see
Figure 2.

![](Files/Opt_ROC.png)

*Figure 2: Process for identifying the optimal comparison process for a
...
variable selection, runs various PTs and CMs, and takes into account
different rules for defining specimen populations (PR).
...
uncorrelated to each other \[4\]. As the IRMS and CE datasets contained
...
area under the ROC curve values for the optimised PT/CM/PR combination
...
timely intelligence \[6\]. In a previous study, it was demonstrated that
a single profiling technique could be prioritised to contribute more
effectively to the generation of timely intelligence \[6\]. In this
...
1.  Collins, M., et al., Illicit drug profiling: the Australian
    ...
    p. 25-32.
2.  Zadora, G., Chemometrics and Statistical Considerations in Forensic
    Science, in Encyclopedia of Analytical Chemistry. 2006, John Wiley &
    Sons, Ltd.
3.  Popovic, A., et al., Review of the most common chemometric
    techniques in illicit drug profiling. Forensic Science
    International, 2019. 302: p. 109911.
4.  Morelato, M., Forensic drug profiling: A tool for intelligence-led
    policing. 2015
5.  Moore, D.S., Chapter 5. Regression, in The Basic Practice of
    Statistics. 2010, Freeman. p. 157.
6.  Morelato, M., et al., The use of organic and inorganic impurities
